...
url = "https://www.soccerstats.com/results.asp?league=chile&pmtype=bygameweek"
data = requests.get(url,time.sleep(2))
soup = BeautifulSoup(data.content)
soup.encode(formatter=None)
div = soup.find("div", class_="tabbertabdefault")
table = div.find("table", id="btable")
f = open("chile1/fixtures.txt", "w")
f.write(str(table))
f.close()
...
url = "https://www.soccerstats.com/results.asp?league=chile&pmtype=bydate"
data = requests.get(url,time.sleep(2))
soup = BeautifulSoup(data.content)
div = soup.find_all("table", id="btable")
f = open("chile1/seasonFixtures.txt", "w")
f.write(str(div[0]))
f.close()
...
url = "https://www.soccerstats.com/results.asp?league=chile_2019&pmtype=bydate"
data = requests.get(url,time.sleep(2))
soup = BeautifulSoup(data.content)
div = soup.find("div", class_="six columns")
table = soup.find("table", id="btable")
f = open("chile1/seasonFixtures2018.txt", "w")
f.write(str(table))
f.close()
